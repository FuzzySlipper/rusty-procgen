...
export function sha256Text(value: string): string {
  const source = UTF8.encode(value);
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (
        h + sum1 + choice + (SHA256_ROUND[index] ?? 0) + (words[index] ?? 0)
      ) >>> 0;
...
export function sha256Json(value: unknown): string {
  ...
  return `sha256:${sha256Text(encoded)}`;
}

export function sha256Serialized(serialized: string): string {
  return `sha256:${sha256Text(serialized)}`;
}

export function fnv1a64Json(value: unknown): string {
  const encoded = JSON.stringify(value);
  if (encoded === undefined) {
    throw new Error('cannot hash a non-JSON value');
  }
  let hash = 0xcbf2_9ce4_8422_2325n;
  for (const byte of UTF8.encode(encoded)) {
    hash ^= BigInt(byte);
    hash = (hash * 0x0000_0100_0000_01b3n) & U64_MASK;
  }
